...
import static com.airbnb.epoxy.media.PlaybackInfo.SCRAP;
import static com.airbnb.epoxy.widget.Common.ORDER_COMPARATOR_INT;
/**
...
final class PlaybackInfoCache extends RecyclerView.AdapterDataObserver {
  ...
  final void onPlayerAttached(ToroPlayer player) {
    ...
    if (cache == null || cache == SCRAP) {
      ...
      cache = container.getPlayerInitializer().initPlaybackInfo(playerOrder);
  ...
  @NonNull final PlaybackInfo getPlaybackInfo(int position) {
    ...
    if (info != null && info == SCRAP) {  // has hot cache, but was SCRAP.
      info = container.getPlayerInitializer().initPlaybackInfo(position);
    }

    Object key = getKey(position);
    return info != null ? info :  //
        key != null ? coldCache.get(key) : container.getPlayerInitializer().initPlaybackInfo(position);
  }
